#!/usr/bin/env python3
"""
Evaluate trained policies on Shadow Hand manipulation tasks.
...
def evaluate_policy(
    ...
    # Load VecNormalize stats if available (saved during training)
    ...
    for ep in iterator:
        ...
        while not done:
            ...
            # Debug output every 50 steps when rendering
            if render:
                time.sleep(0.01)
                ...
                if episode_length % 50 == 0 and hasattr(actual_env, '_get_cube_pos'):
                    ...
                    print(f"    step {episode_length:3d} | base_x={base_x:+.3f} "
                          f"palm=({palm[0]:+.3f},{palm[1]:+.3f},{palm[2]:+.3f}) "
                          f"cube=({cube[0]:+.3f},{cube[1]:+.3f},{cube[2]:+.3f}) "
                          f"palm->cube={palm_to_cube:.3f}m fingers={n_fingers}{tip_str}")
        ...
        if verbose and render:
            status = "pass" if ep_info.get("is_success", False) else "fail"
...
def evaluate_all_tasks(
    ...
    """Evaluate all tasks and print a summary table."""
    all_results = []
...
def main():
    ...
    if args.task.lower() == "all":
        if args.render:
            print("Render mode disabled for 'all' tasks evaluation")
        evaluate_all_tasks(
